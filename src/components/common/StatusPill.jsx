...
const StatusPill = ({ label, status }) => (
  <span
    className={`inline-flex items-center gap-1 px-2 py-0.75 rounded-xl text-[11px] whitespace-nowrap ${statusStyles[status] || ""}`}
  >
    <span
      className={`w-1.25 h-1.25 rounded-full shrink-0 ${dotStyles[status] || ""}`}
    />
    {label}
  </span>
);
