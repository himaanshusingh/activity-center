...
const WorkflowStep = ({ name, status, state }) => (
  <div className="border border-border rounded-lg p-3 flex items-center gap-3 bg-card">
    <div
      className={`w-7.5 h-7.5 rounded-full flex items-center justify-center text-sm shrink-0 font-semibold ${stateStyles[state] || ""}`}
    >
    ...
    <div>
      ...
      <div
        className={`text-[11px] text-muted-foreground ${state === "warn" ? "text-ac-amber" : ""}`}
      >
        {status}
      </div>
    </div>
